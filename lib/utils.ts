...
export function cn(...args: ClassValue[]): string {
  return twMerge(cx(...(args as Parameters<typeof cx>)))
}
 ...
 */
export const isExternalLink = (href: string | null | undefined): boolean => {
 ...
 */
export const getDateTimeFormat = (date: string | Date): string => {
 ...
 */
export const dasherize = (text: string | number): string =>
...
export const sortByProperty = cache(
  <T>(arr: T[] | null | undefined, prop: keyof T): T[] | null | undefined => {
    return arr?.sort((a, b) => {
      const itemA = String(a[prop]).toUpperCase()
      const itemB = String(b[prop]).toUpperCase()
 ...
 */
export const getSortedPosts = cache(<T extends PostItem>(posts: T[]): T[] => {
 ...
 */
export const getItemsByYear = <T extends DateItem>(items: T[]): [number, T[]][] => {
  return items.reduce<[number, T[]][]>((acc, item) => {
    const year = new Date(item.date || item.sys?.firstPublishedAt || '').getFullYear()
    const yearArr = acc.find((arr) => arr[0] === year)
    if (!yearArr) {
      acc.push([year, [item]])
    } else {
      yearArr[1].push(item)
